...
const (
	// MaxPktSize returns the size of data. The chunk size must include 4-byte
	// length prefix. This constant is different from MaxSidebandData because
	// we don't include band band number.
...
func newServerConn(c net.Conn) *ServerConn {
	...
	reader := streamio.NewReader(func() ([]byte, error) {
		if !scanner.Scan() {
			...
			// that we allow two kinds of streams: "000bhello world0000" (with
			// trailing 0000) and "000bhelo world" (without trialing 0000).
			// Having optional behaviors like this is a source of complexity.
			// We should not allow 000bhelo world without the trailing 0000.
...
func (cc *ClientConn) CloseWrite() error {
	...
	cc.writeClosed = true
	if err := cc.conn.SetWriteDeadline(time.Now().Add(sidechannelTimeout)); err != nil {
		return err
	}
